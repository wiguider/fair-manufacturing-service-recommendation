...
\begin{abstract}
Cloud manufacturing platforms connect industrial clients with service providers, yet their recommendation systems exhibit systematic popularity bias favoring large manufacturers over economically disadvantaged small and medium enterprises (SMEs)~\cite{zheng2022cloud, lu2020cloud}. While provider-side exposure fairness~\cite{singh2018fairness, aird2025integrating}, fair re-ranking algorithms~\cite{ye2025bankfair, melucci2025three}, and intersectional fairness methods~\cite{gonzalez2025tutorial, cachel2022manirank} have received growing attention, no prior work addresses fairness in manufacturing service recommendation. Conversely, recent advances in manufacturing service matching leverage knowledge graphs and graph neural networks~\cite{li2024building, wang2025mfg_kggcn, li2024capability} but entirely neglect provider equity. We propose a multi-attribute fair re-ranking method that ensures equitable exposure across manufacturer size categories and historically underrepresented U.S.\ geographic regions. Our key contribution is an intersectional fairness deficit score that dynamically quantifies exposure gaps across the cross-product of multiple protected attributes and guides a greedy re-ranking procedure balancing relevance with multi-dimensional equity---extending single-attribute methods such as FA*IR~\cite{zehlike2017fair} and DetConstSort~\cite{geyik2019fairness}, which cannot jointly optimize across intersecting group memberships. Evaluated on the Manufacturing Service Knowledge Graph (MSKG), our method achieves the highest intersectional demographic parity ratio (0.487 vs.\ 0.341 for DetConstSort) while incurring a moderate and controllable relevance loss (NDCG@10 decreases from 0.0586 to 0.0519), governed by a single $\lambda$ parameter. While demonstrated on manufacturing, the approach generalizes to any service marketplace where provider fairness spans multiple attributes. To our knowledge, this is the first work bringing fairness constraints to manufacturing service recommendation.
\end{abstract}
